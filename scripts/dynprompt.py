...
EXT_VER  = "1.3.0"  # bump this when you update to verify load
    ...
    def ui(self, is_img2img):
        with gr.Accordion(f"{EXT_NAME} v{EXT_VER}", open=False):
            ...
            auto_mirror = gr.Checkbox(
                ...
                info="If a __name-mir__ appears in the positive prompt but not in negative, it will be auto-injected into the negative (as the comma-separated 'other' options)."
            )
            gr.Markdown(
        ...
        # IMPORTANT: The order here must match the process() arguments after `p`
        return [enable, wildcard_dir, auto_mirror]

    def process(self, p, enable, wildcard_dir, auto_mirror):
        # Simple banner to confirm this script loaded (check your WebUI console)
        ...
        # Ensure seeds are prepared; SD uses these to size arrays
        fix_seed(p)
        ...
        # Read the raw user inputs ONCE (used both for expansion and token detection)
